...
public:
  ...
  // LLFuzz
  void start_fuzzer();
  void inject_mac_pdu(uint32_t tti);

  void set_llfuzz_config_file(std::string& llfuzz_config_file_) { llfuzz_config_file = llfuzz_config_file_; }
...
private:
  ...
  // LLFuzz
  bool              activeFuzzer = false;
  bool              enableADB  = true;
  targetLayer_t     targetLayer = MAC;
  // ADBController     adbController;
  LLFuzz            llfuzz;
  std::string       llfuzz_config_file = "../config/llfuzz.conf";
  LLFuzz_config_t   llfuzz_config;

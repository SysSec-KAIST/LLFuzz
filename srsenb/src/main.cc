 ...
 ***********************************************************************/
string                   config_file;
string                   llfuzz_config_file;  // LLFuzz config file
...
std::string read_enb_config_path(const std::string& llfuzz_config_file) {
  std::cout << "[LLFuzz] Reading LLFuzz config from file: " << llfuzz_config_file << std::endl;
  std::ifstream file(llfuzz_config_file);
  std::string line;
  ...
  while (std::getline(file, line)) {
      ...
      // Extract the part before any inline comment for parsing
      std::string parse_line = line;
      size_t commentPos = parse_line.find('#');
      if (commentPos != std::string::npos) {
          parse_line = parse_line.substr(0, commentPos);
      }

      std::istringstream is_line(parse_line);
      std::string key;
  ...
  // Check if enb_config exists in the config file
  if (configMap.find("enb_config") == configMap.end()) {
      std::cout << "Error: enb_config not found in " << llfuzz_config_file << std::endl;
      exit(1);
  }

  std::string enb_config_path = configMap["enb_config"];
  std::cout << "[LLFuzz] Found eNB config path: " << enb_config_path << std::endl;

  /* Modify configs in enb.conf based on values in llfuzz.conf */
  // Read entire enb.conf file
  std::ifstream enb_file(enb_config_path);
  if (!enb_file.is_open()) {
    std::cout << "Error: Cannot open " << enb_config_path << std::endl;
    exit(1);
  }

  std::vector<std::string> lines;
  std::string enb_line;
  while (std::getline(enb_file, enb_line)) {
    lines.push_back(enb_line);
  }
  enb_file.close();

  // Create mapping between llfuzz.conf keys and enb.conf keys
  std::unordered_map<std::string, std::string> keyMapping = {
    {"mcc", "mcc"},
    {"mnc", "mnc"},
    {"n_prb", "n_prb"},
    {"tm", "tm"},
  ...
  // Track current section in enb.conf
  std::string current_section = "";
  
  // Modify enb.conf lines
  for (size_t i = 0; i < lines.size(); i++) {
    ...
    if (lines[i][0] == '[') {
      size_t end_bracket = lines[i].find(']');
      if (end_bracket != std::string::npos) {
        current_section = lines[i].substr(1, end_bracket - 1);
        continue;
      }
    }
    ...
    // Split the line into key-value part and comment part if there's a comment
    if (comment_pos != std::string::npos) {
      key_value = lines[i].substr(0, comment_pos);
      comment = lines[i].substr(comment_pos);
    } else {
      key_value = lines[i];
      comment = "";
    }
    ...
    if (equals_pos != std::string::npos) {
      std::string key = trim(key_value.substr(0, equals_pos));
      
      // Handle special case for PCAP filename in [pcap] section
      if (current_section == "pcap" && key == "filename" && configMap.find("enbPCAPFilename") != configMap.end()) {
        lines[i] = key + " = " + configMap["enbPCAPFilename"] + (comment.empty() ? "" : " " + comment);
        continue;
      }

      // Check if this key should be replaced
      for (const auto& mapping : keyMapping) {
        if (key == mapping.second && configMap.find(mapping.first) != configMap.end()) {
          lines[i] = key + " = " + configMap[mapping.first] + (comment.empty() ? "" : " " + comment);
          break;
        }
      }
    }
  ...
  // Write back to enb.conf
  std::ofstream enb_out(enb_config_path);
  if (!enb_out.is_open()) {
    std::cout << "Error: Cannot write to " << enb_config_path << std::endl;
    exit(1);
  }

  for (const auto& line : lines) {
    enb_out << line << std::endl;
  }
  enb_out.close();

  return enb_config_path;
}
...
void parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  ("llfuzz_config", bpo::value<string>(&llfuzz_config_file), "LLFuzz configuration file");

  // position.add_options()
  //   ("config_file", bpo::value< string >(&config_file), "eNodeB configuration file")
  // ;

  // Positional options - make llfuzz_config position 1
  bpo::positional_options_description p;
  p.add("llfuzz_config", 1);
  ...
  if (vm.count("help")) {
    cout << "Usage: " << argv[0] << " [options] llfuzz_config" << endl << endl;
  ...
  // Get eNB config file path from llfuzz.conf
  config_file = read_enb_config_path(llfuzz_config_file);
  args->llfuzz_config_file = llfuzz_config_file; // save llfuzz config file path to set in mac
...
static bool enable_fuzzer = false;
...
static void execute_cmd(metrics_stdout* metrics, srsenb::enb_command_interface* control, const string& cmd_line)
{
  ...
  if (cmd[0] == "t") {
    ...
    if (enable_fuzzer) {
      cout << "Started LLFuzz, enter n to stop sending test cases." << endl;
    } else {
      cout << "Stopped LLFuzz, enter n to restart sending test cases." << endl;
    }
    control->start_fuzzer();
  } else {

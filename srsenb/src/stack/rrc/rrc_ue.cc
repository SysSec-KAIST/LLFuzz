...
void rrc::ue::parse_ul_dcch(uint32_t lcid, srsran::unique_byte_buffer_t pdu)
{
  ...
  switch (ul_dcch_msg.msg.c1().type()) {
    ...
    case ul_dcch_msg_type_c::c1_c_::types::rrc_conn_recfg_complete:
      ...
      printf("\033[34m");
      printf("[RRC] User 0x%x (%d) connected\n", rnti, rnti);
      printf("\033[0m");
      printf("\n");
                                     ...
                                     srsran::const_byte_span      nas_pdu)
{
  ...
  // T_note: code for enabling 16bits of SN in RLC AM
  // printf("[LLFuzz] rrc_config_type value in rrcreconfig: %d\n", (int)(*llfuzz_rrc_reconfig_type));
  if (*llfuzz_rrc_reconfig_type != RLC_NORMAL){
    if (*llfuzz_rrc_reconfig_type == RLC_UM_5BIT_SN){
      // RLC UM 5bit SN
      for (auto& drb: recfg_r8.rr_cfg_ded.drb_to_add_mod_list){
        // pdcp config
        drb.pdcp_cfg_present = true;
        drb.pdcp_cfg.rlc_um_present = true;
        drb.pdcp_cfg.rlc_am_present = false;
        drb.pdcp_cfg.rlc_um.pdcp_sn_size = pdcp_cfg_s::rlc_um_s_::pdcp_sn_size_opts::len12bits;
        // rlc config
        drb.rlc_cfg_present = true;
        drb.rlc_cfg.set_um_bi_dir();
        drb.rlc_cfg.um_bi_dir().dl_um_rlc.sn_field_len = sn_field_len_e::size5;
        drb.rlc_cfg.um_bi_dir().ul_um_rlc.sn_field_len = sn_field_len_e::size5;
        drb.rlc_cfg.um_bi_dir().dl_um_rlc.t_reordering = t_reordering_opts::ms45;

        printf("[LLFuzz] Set RLC 5bit SN\n");
      }
    }else if (*llfuzz_rrc_reconfig_type == PDCP_7BIT_SN){
      for (auto& drb: recfg_r8.rr_cfg_ded.drb_to_add_mod_list){
        drb.pdcp_cfg.rlc_um.pdcp_sn_size = pdcp_cfg_s::rlc_um_s_::pdcp_sn_size_opts::len7bits;
        printf("[LLFuzz] Set PDCP 7bit SN\n");
      }
    }else{
      for (auto& drb: recfg_r8.rr_cfg_ded.drb_to_add_mod_list){
        drb.pdcp_cfg_present = true;
        drb.pdcp_cfg.rlc_um_present = false;
        drb.pdcp_cfg.rlc_am_present = true;
        drb.pdcp_cfg.rlc_am.status_report_required = true;
        drb.rlc_cfg_present = true;

        // this is for pdcp 15bit sn
        if (*llfuzz_rrc_reconfig_type == PDCP_15BIT_SN){
          drb.pdcp_cfg.ext = true;
          drb.pdcp_cfg.pdcp_sn_size_v1130_present = true;  // 11
          printf("[LLFuzz] Set PDCP 15bit SN\n");
        }
        
        // pdcp 18 bit sn
        if (*llfuzz_rrc_reconfig_type == PDCP_18BIT_SN){
          drb.pdcp_cfg.ext = true;
          drb.pdcp_cfg.pdcp_sn_size_v1310_present = true; // 13
          printf("[LLFuzz] Set PDCP 18bit SN\n");
        }

        
        drb.rlc_cfg.set_am();
        drb.rlc_cfg.am().dl_am_rlc.t_reordering.value = t_reordering_opts::ms15; // ms15
        drb.rlc_cfg.am().dl_am_rlc.t_status_prohibit = t_status_prohibit_opts::ms0;

        drb.rlc_cfg.am().ul_am_rlc.t_poll_retx = t_poll_retx_opts::ms50; // ms50
        drb.rlc_cfg.am().ul_am_rlc.poll_pdu = poll_pdu_opts::p8; // p8
        drb.rlc_cfg.am().ul_am_rlc.poll_byte = poll_byte_opts::kb1000; // kb1000
        drb.rlc_cfg.am().ul_am_rlc.max_retx_thres = ul_am_rlc_s::max_retx_thres_opts::t8;
        if (*llfuzz_rrc_reconfig_type == RLC_16BIT_SN || *llfuzz_rrc_reconfig_type == RLC_15BIT_LI || *llfuzz_rrc_reconfig_type == RLC_16BIT_SN_15BIT_LI){ // *llfuzz_rrc_reconfig_type != PDCP_15BIT_SN && *llfuzz_rrc_reconfig_type != PDCP_18BIT_SN
          drb.ext = true;
        }

        if (*llfuzz_rrc_reconfig_type == RLC_10BIT_SN_11BIT_LI){
          printf("[LLFuzz] Set RLC 10bit SN 11bit LI\n");
        }

        // code for enable extended SN RLC AM (16BIT SN)
        if (*llfuzz_rrc_reconfig_type == RLC_16BIT_SN || *llfuzz_rrc_reconfig_type == RLC_16BIT_SN_15BIT_LI){
          asn1::rrc::rlc_cfg_v1310_s rlc_cfg_v13 = {};
          rlc_cfg_v13.dl_extended_rlc_am_sn_r13 = true;
          // rlc_cfg_v13.ul_extended_rlc_am_sn_r13 = true;
          drb.rlc_cfg_v1310 = asn1::copy_ptr<asn1::rrc::rlc_cfg_v1310_s>(new asn1::rrc::rlc_cfg_v1310_s(rlc_cfg_v13));
          printf("[LLFuzz] Set RLC 16bit SN\n");
        }
        
        // code for enable extended LI in RLC AM (15BIT LI)
        if (*llfuzz_rrc_reconfig_type == RLC_15BIT_LI || *llfuzz_rrc_reconfig_type == RLC_16BIT_SN_15BIT_LI){ 
          asn1::rrc::rlc_cfg_v1250_s rlc_cfg_v12 = {};
          rlc_cfg_v12.dl_extended_rlc_li_field_r12 = true;
          // rlc_cfg_v12.ul_extended_rlc_li_field_r12 = true;
          drb.rlc_cfg_v1250 = asn1::copy_ptr<asn1::rrc::rlc_cfg_v1250_s>(new asn1::rrc::rlc_cfg_v1250_s(rlc_cfg_v12));
          printf("[LLFuzz] Set RLC 15bit LI\n");
        }
      }
    }
  }

...
int rrc::add_user(uint16_t rnti, const sched_interface::ue_cfg_t& sched_ue_cfg)
{
  ...
  if (user_it == users.end()) {
    if (rnti != SRSRAN_MRNTI) {
      ...
      u->set_rrc_reconfig_type(llfuzz_rrc_reconfig_type);
...
void rrc::fuzzer_release_ue(uint16_t rnti)
{
  rrc_pdu p = {rnti, LCID_REL_USER, false, nullptr};
  if (not rx_pdu_queue.try_push(std::move(p))) {
    printf("[RRC] Cannot add release UE \n");
  }
}

void rrc::fuzzer_send_paging(uint32_t ueid, uint8_t mmec, uint8_t* m_tmsi_)
{
  srsran::const_byte_span m_tmsi(m_tmsi_, 4);
  pending_paging->add_tmsi_paging(ueid, mmec, m_tmsi);
}
...
void rrc::rem_user(uint16_t rnti)
{
  ...
  if (user_it != users.end()) {
    ...
    // printf("[RRC] Disconnecting RNTI 0x%x (%d)\n", rnti, rnti);

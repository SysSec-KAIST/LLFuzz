...
uint32_t rlc::get_sequence_number(uint16_t rnti, uint32_t lcid){
  uint32_t ret = 0;
  if (users.count(rnti)) {
    ret = users[rnti].rlc->get_sequence_number(lcid);
  }
  return ret;
}
...
void rlc::mac_control_rlc_write_sdu(uint16_t rnti, uint32_t lcid, srsran::unique_byte_buffer_t sdu)
{
  write_sdu(rnti, lcid, std::move(sdu));
}

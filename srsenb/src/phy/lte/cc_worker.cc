                        ...
                        srsran_mbsfn_cfg_t*                  mbsfn_cfg)
{
  ...
  if (dl_sf_cfg.sf_type == SRSRAN_SF_NORM) {
    ...
    bool sendThisSF = dl_grants.sendThisSF;
    if (sendThisSF ) {
      // std::cout << "SF: " << targetTTI/10 << "." << targetTTI%10 << " -- Send this SF in cc worker" << std::endl;
      encode_pdcch_dl_fuzzer(dl_grants.pdsch, dl_grants.pdsch_fuzzer, dl_grants.nof_grants);
    }else{           
  ...
  bool sendThisSF_UL = ul_grants.sendThisSF_UL;
  if (sendThisSF_UL) {
    encode_pdcch_ul_fuzzer(ul_grants.pusch_fuzzer, ul_grants.nof_grants);
  } else {
...
int cc_worker::encode_pdcch_dl_fuzzer(stack_interface_phy_lte::dl_sched_grant_t* orin_grants, stack_interface_phy_lte::dl_sched_grant_t* fuzzer_grants, uint32_t nof_grants)
{
  for (uint32_t i = 0; i < nof_grants; i++) {
    uint16_t rnti = fuzzer_grants[i].dci.rnti;
    if (rnti) {
      srsran_dci_cfg_t dci_cfg = {};

      if (phy->ue_db.get_dci_dl_config(fuzzer_grants[i].dci.rnti, cc_idx, dci_cfg) < SRSRAN_SUCCESS) {
        Error("Error retrieving DCI DL configuration for RNTI %x, CC %d", fuzzer_grants[i].dci.rnti, cc_idx);
        continue;
      }

      // Detect if the DCI location is in common SS, if that is the case, flag it as common SS
      // This makes possible UE specific DCI fields to be disabled, so it uses a fallback DCI size
      if (SRSRAN_RNTI_ISUSER(fuzzer_grants[i].dci.rnti) && fuzzer_grants[i].dci.format == SRSRAN_DCI_FORMAT1A) {
        if (srsran_enb_dl_location_is_common_ncce(&enb_dl, &fuzzer_grants[i].dci.location) &&
            phy->ue_db.is_pcell(fuzzer_grants[i].dci.rnti, cc_idx)) {
          srsran_dci_cfg_set_common_ss(&dci_cfg);
        }
      }

      if (srsran_enb_dl_put_pdcch_dl(&enb_dl, &dci_cfg, &fuzzer_grants[i].dci)) {
        ERROR("Error putting PDCCH %d", i);
        return SRSRAN_ERROR;
      }
      bool isUserRNTI = SRSRAN_RNTI_ISUSER(rnti);
      if (isUserRNTI){
        std::cout << "[PHY] SF: " << dl_sf.tti/10 << "." << dl_sf.tti%10 << " Finished sending DCI test cases" << std::endl << std::endl;
      }

      if (LOG_THIS(rnti) and logger.info.enabled()) {
        // Logging
        char str[512];
        srsran_dci_dl_info(&fuzzer_grants[i].dci, str, 512);
        logger.info("PDCCH: cc=%d, rnti=0x%x, %s, tti_tx_dl=%d", cc_idx, fuzzer_grants[i].dci.rnti, str, tti_tx_dl);
      }
    }
    // if (SRSRAN_RNTI_ISUSER(fuzzer_grants[i].dci.rnti)) {*nof_dci = *nof_dci + 1;} //increase total number of dci
  }
  return 0;
}

int cc_worker::encode_pdcch_ul_fuzzer(stack_interface_phy_lte::ul_sched_grant_t* grants, uint32_t nof_grants)
{
  for (uint32_t i = 0; i < nof_grants; i++) {
    if (grants[i].needs_pdcch) {
      srsran_dci_cfg_t dci_cfg = {};
      ...
      bool isUserRNTI = SRSRAN_RNTI_ISUSER(grants[i].dci.rnti);
      if (isUserRNTI){
        std::cout << "[PHY] SF: " << dl_sf.tti/10 << "." << dl_sf.tti%10 << " Finished sending DCI UL test cases" << std::endl << std::endl;
      }

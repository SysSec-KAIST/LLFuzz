...
public:
  ...
  /* MAC call RLC to inject test case to RLC 
   * in the interface between PDCP and RLC. */
  virtual void mac_control_rlc_write_sdu(uint16_t rnti, uint32_t lcid, srsran::unique_byte_buffer_t sdu) = 0;
  virtual uint32_t get_sequence_number(uint16_t rnti, uint32_t lcid) = 0;  
};

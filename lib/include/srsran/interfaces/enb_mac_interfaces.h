...
public:
  ...
  typedef struct {
    ...
    // LLFuzz
    bool             sendThisSF = false;          // decide by fuzzer if test cases should be sent this SF
    dl_sched_grant_t pdsch_fuzzer[MAX_GRANTS];    // PDCCH grant from fuzzer, CC worker will read this one if sendThisSF is true
  } dl_sched_t;
  ...
  typedef struct {
    ...
    uint32_t         nof_phich;
    // LLFuzz
    ul_sched_grant_t pusch_fuzzer[MAX_GRANTS];
    bool             sendThisSF_UL = false; // decide by fuzzer if test cases should be sent this SF   
  } ul_sched_t;
